...
namespace PoeHUD.Hud.Health
{
	public class HealthBarRenderer : HUDPluginBase, EntityListObserver
	{
		...
		public override void Render(RenderingContext rc, Dictionary<UiMountPoint, Vec2> mountPoints)
		{
			...
			float clientWidth = this.model.Window.ClientRect().W / 2560f;
			float clientHeight = this.model.Window.ClientRect().H / 1600f;
			foreach (List<Healthbar> bars in this.healthBars)
			{
				...
				{
					RenderHealthBar(rc, model.Internal.IngameState.Camera, current, clientWidth, clientHeight);
				}
			}
		}
